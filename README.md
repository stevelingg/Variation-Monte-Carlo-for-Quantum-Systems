...
- **Metropolis–Hastings MCMC** sampling from $|\psi(x;\theta)|^2$ using a **log-density** for numerical stability
- **Finite-difference Laplacian** (2nd / 4th order) to build the kinetic term and local energy
- **Correlation-aware uncertainty** via integrated autocorrelation time $\tau_{\mathrm{int}}$ and **effective sample size (ESS)**
...
- $\tau_{\mathrm{int}}$ and ESS consistency on an AR(1) process
...
MCMC samples are correlated, so the iid standard error $\sigma/\sqrt{N}$ underestimates uncertainty.  
This code estimates $\tau_{\mathrm{int}}$ and uses:

$$
\mathrm{ESS} = \frac{N}{2\tau_{\mathrm{int}}}
$$

$$
\mathrm{SE}(\langle E\rangle) = \sqrt{\frac{\mathrm{Var}(E_L)}{\mathrm{ESS}}}
$$
...
- **Known benchmarks**: HO energies can be compared against the analytic values ($E_n = n + \tfrac{1}{2}$ in chosen units)
...
- The framework assumes **real-valued** $\psi$ **in the toy systems**; sign changes are handled carefully in divisions
